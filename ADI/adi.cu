...
#define A_i(i, j, k) A[((j) * nz + (k)) * nx + (i)]
#define A_j(i, j, k) A[((k) * nx + (i)) * ny + (j)]
#define A_k(i, j, k) A[((i) * ny + (j)) * nz + (k)]
...
__global__ void function_i(double *A) {
    ...
    if (threadIdx.x == 0)
        ...
        if (i < nx)
            if (j < ny)
                if (k < nz)
                    temp_i(threadIdx.x, threadIdx.y, threadIdx.z) = A_i(i, j, k) / 4;
    ...
    if (i < nx - 1)
        if (j < ny)
            if (k < nz)
                temp_i(threadIdx.x, threadIdx.y, threadIdx.z) += A_i(i + 1, j, k) / 2;
    ...
    if ((i > 0) && (i < nx - 1))
        if ((j > 0) && (j < ny - 1))
            if ((k > 0) && (k < nz - 1))
               A_i(i, j, k) = temp_i(threadIdx.x, threadIdx.y, threadIdx.z) + val_i[j][k] / (1 << (threadIdx.x + 1));
    __syncthreads();
...
__global__ void function_j(double *A) {
    ...
    if (threadIdx.y == 0)
        ...
        if (i < nx)
            if (j < ny)
                if (k < nz)
                    temp_j(threadIdx.x, threadIdx.y, threadIdx.z) = A_j(i, j, k) / 4;
    ...
    if (i < nx)
        if (j < ny - 1)
            if (k < nz)
                temp_j(threadIdx.x, threadIdx.y, threadIdx.z) += A_j(i, j + 1, k) / 2;
    ...
    if ((i > 0) && (i < nx - 1))
        if ((j > 0) && (j < ny - 1))
            if ((k > 0) && (k < nz - 1))
               A_j(i, j, k) = temp_j(threadIdx.x, threadIdx.y, threadIdx.z) + val_j[i][k] / (1 << (threadIdx.y + 1));
    __syncthreads();
...
__global__ void function_k(double *A, double *eps) {
    ...
    if (threadIdx.z == 0)
        ...
        if (i < nx)
            if (j < ny)
                if (k < nz)
                    temp_k(threadIdx.x, threadIdx.y, threadIdx.z) = A_k(i, j, k) / 4;
    ...
    if (i < nx)
        if (j < ny)
            if (k < nz - 1)
                temp_k(threadIdx.x, threadIdx.y, threadIdx.z) += A_k(i, j, k + 1) / 2;
...
__global__ void rotate(double *A) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    int j = blockIdx.y * blockDim.y + threadIdx.y;
    int k = blockIdx.z * blockDim.z + threadIdx.z;
...
int main(int argc, char *argv[])
{
    ...
    if (GPU) {
        ...
        dim3 gridDim_r = dim3(nx / 32 + 1, ny / 4 + 1, nz / 4 + 1);
